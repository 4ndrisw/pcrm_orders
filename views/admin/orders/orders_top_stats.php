...
<div id="stats-top" class="hide">
    <div id="orders_total"></div>

    <div class="_filters _hidden_inputs hidden">
        <?php
            if (isset($orders_sale_agents)) {
                foreach ($orders_sale_agents as $agent) {
            ...
            if (isset($order_statuses)) {
                foreach ($order_statuses as $_status) {
                    ...
                    echo form_hidden('orders_' . $_status, $val);
                }
            }
            if (isset($orders_years)) {
                foreach ($orders_years as $year) {
    ...
    <div class="quick-top-stats">
        <dl
            ...
            <?php foreach ($order_statuses as $status) {
                $percent_data = get_orders_percent_by_status(
                    $status,
                    (isset($project) ? $project->id : null)
                ); ?>
            <div class="tw-px-3 tw-py-4 sm:tw-p-4">
                <dt class="tw-font-medium text-<?php echo order_status_color_class($status); ?>">
                    ...
                    <div class="tw-flex tw-items-baseline tw-text-base tw-font-semibold tw-text-primary-600">
                        ...
                        <span class="tw-ml-2 tw-text-sm tw-font-medium tw-text-neutral-500">
                            <a href="#" data-cview="orders_<?php echo $status; ?>"
                                onclick="dt_custom_view('orders_<?php echo $status; ?>','.table-orders','orders_<?php echo $status; ?>',true); return false;">
                                <?php echo _l('view'); ?>
                            </a>
                        </span>
                    </div>
